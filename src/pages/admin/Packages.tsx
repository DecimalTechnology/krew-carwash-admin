    ...
    return (
        <div className="space-y-4">
            ...
            <div className="w-full flex flex-wrap justify-between items-center gap-4 mt-4">
                ...
                <button onClick={() => setAddModalOpen(true)} className="bg-brand-500 hover:bg-brand-600 text-white px-4 py-2 rounded-lg transition">
                    Add New Package
                </button>
            </div>
            ...
            <div className="flex w-full flex-row justify-between mt-8 mb-8">
                ...
                <ExportAndFilter packageId={packageId} />
            </div>
                        ...
                        <Table>
                            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                                <TableRow>
                                    ...
                                    <TableCell className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400 select-none">Frequency</TableCell>
                                    <TableCell className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400 select-none">Description</TableCell>
                                    <TableCell className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400 select-none">Pricing</TableCell>
                            ...
                            <TableBody>
                                {packages.map((pkg, index) => (
                                    <TableRow key={pkg._id}>
                                        ...
                                        <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">{pkg.name}</TableCell>
                                                ...
                                                <div className="space-y-1">
                                                    {pkg.basePrices.map((bp, idx) => (
                                                        ...
                                                        >
                                                            ...
                                                            <span className="text-xs font-semibold text-brand-600 dark:text-brand-400 whitespace-nowrap">{bp.price.toFixed(0)} AED</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            ) : (
                                        ...
                                        <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">
                                            <Switch checked={pkg.isActive} onChange={() => handleUpdate({ isActive: !pkg.isActive }, pkg._id)} />
                                        </TableCell>
                                        <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">{new Date(pkg.createdAt).toLocaleDateString()}</TableCell>
                                        <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">
            ...
            {addModalOpen && <AddPackageModal onClose={() => setAddModalOpen(false)} isOpen={addModalOpen} onSuccess={() => setFetch(!fetch)} />}

            {editModalOpen && selectedPackage && <EditPackageModal package={selectedPackage} onClose={() => setEditModalOpen(false)} isOpen={editModalOpen} onUpdate={handleEditUpdate} />}

            {viewModalOpen && selectedPackage && <ViewPackageModal package={selectedPackage} onClose={() => setViewModalOpen(false)} />}
