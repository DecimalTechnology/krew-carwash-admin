...
import { TrendingUp, Users, Car, DollarSign, Calendar, MapPin, Star, Droplet, CheckCircle } from "lucide-react";
...
function Dashboard() {
    ...
    const topLocations = useMemo(() => {
        return (dashboardData?.topLocations || [])
            .map((l: any) => ({
                name: l.name,
                bookings: l.bookings,
    ...
    const topCleaners = useMemo(() => {
        return (dashboardData?.topCleaners || [])
            .map((c: any) => ({
                name: c.name,
    ...
    const serviceBreakdown = useMemo(() => {
        ...
        return (dashboardData?.serviceBreakdown || [])
            .map((s: any) => ({
                service: s.service,
                bookings: s.bookings,
    ...
    return (
        <div className="space-y-6 p-4 md:p-6">
            ...
            {/* Stats Grid */}
            {loading && <div className="p-4 bg-white rounded-md text-sm text-gray-500">Loading dashboard...</div>}
                ...
                {stats.map((stat, index) => {
                    ...
                    return (
                      ...
                      <span className="text-sm text-gray-500 dark:text-gray-400">vs last month</span>
                                    ...
                                    <div className="p-3 bg-brand-500 rounded-lg group-hover:scale-110 transition-transform">
                    ...
                    <div className="p-6 space-y-4">
                        {topLocations.map((location: LocationRow, index: number) => (
                            <div key={index} className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="flex items-center justify-center w-8 h-8 rounded-full bg-brand-500 text-white text-xs font-bold">{index + 1}</div>
                                        <div>
                                ...
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                    <div className="bg-brand-500 h-2 rounded-full transition-all duration-500" style={{ width: `${(location.bookings / 500) * 100}%` }}></div>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="p-6 space-y-4">
                        {serviceBreakdown.map((service: ServiceRow, index: number) => (
                            <div key={index} className="space-y-2">
                                ...
                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                                    <div className="bg-brand-500 h-2 rounded-full transition-all duration-500" style={{ width: `${service.percentage}%` }}></div>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-brand-500 rounded-xl shadow-lg p-6 text-white hover:shadow-xl transition-shadow">
                    ...
                    <h3 className="text-xl font-semibold mb-2">Manage Building</h3>
                    <p className="text-white/90 text-sm mb-4">Create a new building for your customers</p>
                    <button onClick={()=>navigate("/add-building")} className=" px-4 py-2 bg-white text-[#5DB7AE] rounded-lg hover:bg-gray-100 transition font-medium">New Building</button>
                </div>
                ...
                <div className="bg-brand-500 rounded-xl shadow-lg p-6 text-white hover:shadow-xl transition-shadow">
                    ...
                    <button onClick={()=>navigate("/cleaners")} className="px-4 py-2 bg-white text-[#5DB7AE] rounded-lg hover:bg-gray-100 transition font-medium">View Cleaners</button>
                </div>
                ...
                <div className="bg-brand-500 rounded-xl shadow-lg p-6 text-white hover:shadow-xl transition-shadow">
                    ...
                    <button className="px-4 py-2 bg-white text-[#5DB7AE] rounded-lg hover:bg-gray-100 transition font-medium">View Reports</button>
                </div>
            </div>
        </div>
    );
