const dummyProfilePic =
    "data:image/png;base64,";
import { SearchIcon } from "lucide-react";
import { useEffect, useState } from "react";
import { getChats } from "../../api/admin/chatService";
import { useDispatch } from "react-redux";
import { setChats, setSelectedChat } from "../../features/chatSlice";
import { IRootState } from "../../app/store";
import { useSelector } from "react-redux";

function LeftChatPannel() {
    const [search, setSearch] = useState("");
    const [tabs, setTabs] = useState("all");
    const dispatch = useDispatch();

    // Get the selected chat from Redux store
    const selectedChat = useSelector((state: any) => state?.chat?.selectedChat);
    const chats = useSelector((state: IRootState) => state?.chat?.chats);

    useEffect(() => {
        const fetchChats = async () => {
            const res = await getChats({ status: tabs, search: search });
            dispatch(setChats(res.data));
        };
        fetchChats();
    }, [dispatch, tabs, search]);

    const handleChatClick = (chat: any) => {
        dispatch(setSelectedChat(chat));
    };

    return (
        <div className="w-1/3 flex flex-col bg-white border-r border-gray-200 h-full">
            {/* Header & Search */}
            <div className="flex-shrink-0 p-4 border-b border-gray-100 bg-white">
                <h2 className="text-xl font-bold text-gray-800 mb-4">Messages</h2>

                {/* Search */}
                <div className="relative mb-4">
                    <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                    <input
                        type="text"
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        placeholder="Search cleaner or issue..."
                        className="w-full pl-9 pr-3 py-2 bg-gray-100 border-none rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-300"
                    />
                </div>

                {/* Filter Tabs */}
                <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg">
                    <div
                        onClick={() => setTabs("all")}
                        className={`cursor-pointer flex-1 py-1.5 text-xs font-semibold rounded-md text-center ${tabs === "all" ? "bg-teal-500 text-white" : "bg-white text-gray-800"} shadow-sm`}
                    >
                        All
                    </div>
                    <div
                        onClick={() => setTabs("open")}
                        className={`cursor-pointer flex-1 py-1.5 text-xs font-semibold rounded-md text-center ${tabs === "open" ? "bg-teal-500 text-white" : "text-gray-500"}`}
                    >
                        Open
                    </div>
                    <div
                        onClick={() => setTabs("resolved")}
                        className={`cursor-pointer flex-1 py-1.5 text-xs font-semibold rounded-md text-center ${tabs === "resolved" ? "bg-teal-500 text-white" : "text-gray-500"}`}
                    >
                        Resolved
                    </div>
                </div>
            </div>

            {/* Chat List */}
            <div className="flex-1 overflow-y-auto">
                {chats?.map((chat: any) => {
                    const isSelected = selectedChat?._id === chat?._id;

                    return (
                        <div
                            onClick={() => handleChatClick(chat)}
                            className={`cursor-pointer w-full flex items-start p-4 border-b border-gray-50 transition-colors duration-200 ${
                                isSelected ? "bg-teal-100 border-l-4 border-l-teal-500" : chat?.status === "open" ? "bg-teal-50 hover:bg-teal-100" : "bg-gray-50 hover:bg-gray-100"
                            }`}
                            key={chat?._id}
                        >
                            <div className="relative mr-3 flex-shrink-0">
                                <div className="w-10 h-10 rounded-full bg-gray-400 flex items-center justify-center text-white font-semibold border-2 border-white">
                                    {chat?.cleaner?.name?.charAt(0).toUpperCase() || "U"}
                                </div>

                                <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 ${chat?.issueResolved == false ? "bg-red-500" : "bg-green-500"} border-2 border-white rounded-full`}></span>
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-baseline mb-1">
                                    <h3 className={`text-sm truncate font-bold ${isSelected ? "text-teal-800" : "text-gray-900"}`}>{chat?.cleaner?.name}</h3>
                                    <span className={`text-xs whitespace-nowrap ml-2 ${isSelected ? "text-teal-600" : "text-gray-400"}`}>
                                        {chat?.updatedAt ? new Date(chat?.updatedAt).toLocaleTimeString() : ""}
                                    </span>
                                </div>
                                <p className={`text-xs truncate mb-1 font-medium ${isSelected ? "text-gray-700" : "text-gray-500"}`}>{chat?.issue}</p>
                                <p className={`text-xs truncate ${isSelected ? "text-gray-600" : "text-gray-400"}`}>{chat?.lastMessage || "No message yet"}</p>
                            </div>

                            {/* Optional: Add a selected indicator */}
                            {isSelected && (
                                <div className="ml-2 flex items-center">
                                    <div className="w-2 h-2 bg-teal-500 rounded-full"></div>
                                </div>
                            )}
                        </div>
                    );
                })}
            </div>
        </div>
    );
}

export default LeftChatPannel;
