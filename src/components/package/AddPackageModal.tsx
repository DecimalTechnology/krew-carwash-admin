...
interface IFormErrors {
    ...
    description?: string;
    basePrices?: string;
    individualBasePrices?: Record<number, string>; // New: individual field errors
}
...
function AddPackageModal({ isOpen, onClose, onSuccess }: IProps) {
    ...
    const [fieldTouched, setFieldTouched] = useState<Record<number, { vehicleType: boolean; price: boolean }>>({});
    ...
    const validateBasePriceField = (index: number, field: "vehicleType" | "price", value: string | number): string => {
        if (field === "vehicleType") {
            ...
            if (price > 10000) return "Price must be less than 10000 AED";
            return "";
        }
        return "";
    };
    ...
    const handleBasePriceChange = (index: number, field: "vehicleType" | "price", value: string | number) => {
        ...
        const fieldError = validateBasePriceField(index, field, value);
        setErrors(prev => ({
            ...prev,
            individualBasePrices: {
                ...prev.individualBasePrices,
                [index]: fieldError
            }
        }));
    ...
    const handleBasePriceBlur = (index: number, field: "vehicleType" | "price") => {
        ...
        const value = basePrices[index][field];
        const fieldError = validateBasePriceField(index, field, value);
        ...
        } else if (errors.individualBasePrices && errors.individualBasePrices[index]) {
            const newIndividualErrors = { ...errors.individualBasePrices };
            delete newIndividualErrors[index];
            setErrors(prev => ({
                ...prev,
                individualBasePrices: newIndividualErrors
            }));
        }
    };
    ...
    const removeBasePrice = (index: number) => {
        if (basePrices.length > 1) {
            ...
            // Remove individual error for this index
            if (errors.individualBasePrices && errors.individualBasePrices[index]) {
                const newIndividualErrors = { ...errors.individualBasePrices };
                delete newIndividualErrors[index];
                setErrors(prev => ({
                    ...prev,
    ...
    const validateForm = (): boolean => {
        ...
        const individualErrors: Record<number, string> = {};
        let hasErrors = false;
        ...
        // Validate each base price field individually
        basePrices.forEach((bp, index) => {
            const vehicleTypeError = validateBasePriceField(index, "vehicleType", bp.vehicleType);
            const priceError = validateBasePriceField(index, "price", bp.price);
            
            if (vehicleTypeError || priceError) {
                individualErrors[index] = vehicleTypeError || priceError;
                hasErrors = true;
            }
        });
        
        // Validate base prices array (duplicates, etc.)
        const basePricesError = validateBasePrices(basePrices);
        if (basePricesError) {
            newErrors.basePrices = basePricesError;
            hasErrors = true;
        }
        
        if (Object.keys(individualErrors).length > 0) {
            newErrors.individualBasePrices = individualErrors;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Mark all base price fields as touched
        const newFieldTouched: Record<number, { vehicleType: boolean; price: boolean }> = {};
        basePrices.forEach((_, index) => {
            newFieldTouched[index] = { vehicleType: true, price: true };
        });
        setFieldTouched(newFieldTouched);
    ...
    // Helper to check if individual base price field should show error
    const showBasePriceError = (index: number): boolean => {
        const isTouched = fieldTouched[index]?.vehicleType || fieldTouched[index]?.price;
        return !!isTouched && !!errors.individualBasePrices?.[index];
    };
            ...
            >
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {basePrices.map((basePrice, index) => {
                                    ...
                                    const fieldError = errors.individualBasePrices?.[index];
                                    const hasError = showBasePriceError(index);
                                    
                                    return (
                                        <div key={index} className={`flex gap-3 items-start p-4 rounded-lg border ${
                                            hasError 
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <select
                                                    ...
                                                    onBlur={() => handleBasePriceBlur(index, "vehicleType")}
                                                    required
                                                ...
                                                >
                                                    ...
                                                    {selectedVehicle && !availableVehicleTypes.some(vt => vt._id === selectedVehicle._id) && (
                                                        <option value={selectedVehicle._id}>
                                                            {selectedVehicle.name} (selected)
                                                        </option>
                                                    )}
                                                </select>
                                                {hasError && fieldError && basePrice.vehicleType === "" && (
                                                    <p className="text-xs text-red-500 mt-1">{fieldError}</p>
                                                )}
                                                {availableVehicleTypes.length === 0 && !basePrice.vehicleType && (
                                                    <p className="text-xs text-red-500 mt-1">No more vehicle types available</p>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <input
                                                    ...
                                                    onBlur={() => handleBasePriceBlur(index, "price")}
                                                    required
                                                    placeholder="0.00"
                                                    className={`w-full border rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all duration-200 ${
                                                        hasError 
                                                ...
                                                />
                                                {hasError && fieldError && (basePrice.price === "" || basePrice.price === 0) && (
                                                    <p className="text-xs text-red-500 mt-1">{fieldError}</p>
                                                )}
                            ...
                            <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 dark:text-gray-400">{formData.isActive ? "This package will be active" : "This package will be inactive"}</p>
                                    </div>
                ...
                <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3 flex-shrink-0">
                    ...
                    <button
                        ...
                        className={`px-5 py-2.5 rounded-lg font-semibold transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] ${
                      
                               
                                "bg-brand-500 hover:bg-brand-600 dark:bg-brand-600 dark:hover:bg-brand-700 text-white hover:shadow-lg"
                        }`}
                    >
